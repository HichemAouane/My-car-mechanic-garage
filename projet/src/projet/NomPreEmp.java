...
import java.awt.event.ActionEvent;
	...
	public class NomPreEmp extends JFrame{
		...
		public static void main(String[] args) {
			EventQueue.invokeLater(new Runnable() {
				public void run() {
					try {
						NomPreEmp frame = new NomPreEmp(connection1,stmt1);
		...
		public NomPreEmp(Connection connection ,Statement stmt) {
			setTitle(" les employers qui sont presents sur plus de x intervention ");
			...
			btnAfficher.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					try {
						rs = stmt1.executeQuery("SELECT nomemp, prenomemp FROM employe WHERE EXISTS (SELECT * FROM intervenants WHERE intervenants.numemploye = employe.numemploye GROUP BY numemploye HAVING COUNT(*) >"+ textField_1.getText()+")");
			...
			JLabel lblVeuillezSvpInserer = new JLabel("les employers qui sont presents sur plus de x intervention:");
